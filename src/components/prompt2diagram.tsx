...
const Prompt2Diagram = () => {
  ...
  // UPDATED: Kroki encoding function (replaces PlantUML encoding)
  const generateDiagramUrl = (plantUMLCode) => {
    try {
      // Clean the PlantUML code first
      const cleanCode = plantUMLCode.trim();
      
      // Use URL-safe base64 encoding for Kroki
      const encoded = btoa(unescape(encodeURIComponent(cleanCode)))
        .replace(/\+/g, '-')
        .replace(/\//g, '_')
        .replace(/=+$/, ''); // Remove padding
        
      console.log('Base64 encoded:', encoded); // Debug log
  ...
  // Generate PlantUML code (unchanged - your logic is perfect)
  ...
  // UPDATED: Generate diagram using Kroki
  const handleGenerateDiagram = async () => {
    ...
    try {
      ...
      console.log('Generated PlantUML code:', umlCode); // Debug log
      setPlantUMLCode(umlCode);
      
      const imageUrl = generateDiagramUrl(umlCode);
  ...
  // UPDATED: Export functions using Kroki
  const exportPNG = async () => {
    ...
    try {
      // Use same encoding as diagram generation
      const encoded = btoa(unescape(encodeURIComponent(plantUMLCode)))
        .replace(/\+/g, '-')
        .replace(/\//g, '_')
        .replace(/=+$/, '');
      const pngUrl = `https://kroki.io/plantuml/png/${encoded}`;
  ...
  // Add connection (unchanged - your logic is perfect)
  const addConnection = () => {
  ...
  // Auto-generate diagram when connections change (unchanged)
  useEffect(() => {
    if (connections.length > 0) {
